...
**Платная подписка Дом.ру для видео не нужна.** Если она есть, ролик вырезается из
облачного архива оператора (`/video?TS=`) в 1080p. Если нет, прокси сам это
определяет и переключается на **live-буфер**: держит соединение с лёгким потоком
камеры (960×528, ~0.45 Мбит/с) и хранит последние 20 с в памяти, поэтому секунды до
звонка тоже попадают в ролик. На диск ничего не пишется, вся обработка происходит в ОЗУ. `DOMRU_TELEGRAM_VIDEO=buffer`
включает буфер сразу, без обращения к архиву. FLV ремуксится в MP4 в памяти, без
ffmpeg. Подписка нужна только для просмотра камер соседних подъездов на главной.
